...
class ProbabilisticDecoder(nn.Module):
    """
    Neural network ProbabilisticDecoder module that creates a feedforward network for decoding latent representations.
    ...
    Methods:
    - __init__(self, latent_dim, hidden_dim, output_dim, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout=0, debug=False): Initializes the ProbabilisticDecoder.
    - forward(self, x): Performs a forward pass through the ProbabilisticDecoder.
    """

    def __init__(self, latent_dim, hidden_dim, output_dim, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout=0, debug=False):
        """
        Initializes the ProbabilisticDecoder module.
    ...
    def forward(self, x):
        """
        Performs a forward pass through the ProbabilisticDecoder.

        Args:
        - x (Tensor): Input data.

        Returns:
        - x_hat (Tensor): Decoded output data with sigmoid activation applied.
        """
