...
from src.network.multimodal_customics import MultimodalCustOMICS
...
def train(task, cohorts, sources, split, device, num_classes=4,
    ...
    if cohorts == 'PANCAN':
        ...
        if cohorts == 'TCGA-BRCA':
            label = 'PAM50'
        elif cohorts == 'TCGA-LUAD' and task == 'classification':
            label = 'risk'
        else:
            label = 'Tumor_purity'
        event = 'status'
    ...
    omics_df, clinical_df, omics_train, omics_val, omics_test, lt_samples, hallmarks, hallmarks_loc, hallmarks_dim = prepare_dataset(
        cohorts, sources, split, label=label)
    ...
    for i, source in enumerate(sources):
        source_params['omics'][source] = {'input_dim': hallmarks_dim[source], 'hidden_dim': hidden_dim,
                                 'latent_dim': rep_dim, 'norm': True, 'dropout': 0.2}
    source_params['WSI'] = {'input_dim': 1024, 'hidden_dim': [6,6],
                            'latent_dim': rep_dim, 'pooling': 'attention', 'dropout': 0.2, 'n_clusters':10,
                            'WSI_path': '../data/TCGA/{}/WSI_preprocess/pretrained/cluster_graphs/'.format(cohorts)}
    train_params = {'switch': 1, 'lr': 1e-3}

    model = MultimodalCustOMICS(source_params=source_params, central_params=central_params, classif_params=classif_params,
                      surv_params=surv_params, train_params=train_params, device=device, pathways=hallmarks, hallmarks_loc=hallmarks_loc).to(device)
    ...
    if cohorts != 'PANCAN' and task == 'classification':
        model.explain(lt_samples, omics_df, clinical_df,
                      'RNAseq', 'high', label, device, get_names=True, sample=False)
    if task == 'survival':
        #print(model.predict_survival(omics_test))
        model.stratify(omics_df=omics_df, clinical_df=clinical_df,
                       event=event, surv_time=surv_time, save_plot=True, filename='PANCAN_survival')
    return metric
