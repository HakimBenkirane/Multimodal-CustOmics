...
def extract_hallmarks(omics_df):
    with open('hallmarks.json', 'r') as f:
        hallmarks = json.load(f)
    return {key: {source: list(set(value['geneSymbols']).intersection(set(omic_df.columns))) for source, omic_df in omics_df.items()} for key, value in hallmarks.items()}
...
def load_graph(graph_path, slide_name, cluster=0):
    with open(graph_path + slide_name, 'rb') as f:
        graph = pickle.load(f)
    if cluster:
...
def filter_genes(omics_df, hallmark):
    new_omics_df = {}
    hallmarks = extract_hallmarks()
    for key, df in omics_df.items():
        new_omics_df[key] = df.loc[:, list(set(hallmarks[hallmark]).intersection(set(df.columns)))]
    return new_omics_df

def get_hallmarks_loc(omics_df, hallmarks):
    hallmarks_loc = {}
    for hallmark, gene_set in hallmarks.items():
            hallmarks_loc[hallmark] = {source: [omics_df[source].columns.get_loc(gene) for gene in gene_set[source]] for source in gene_set.keys()}
    return hallmarks_loc


def read_data(cohort, omic_sources, label):
    omics_df = {}
    cnv_path = '../data/TCGA/{}/Omics/CNV/{}.gistic.tsv'.format(cohort,cohort)
    rna_path = '../data/TCGA/{}/Omics/RNAseq/{}.htseq_fpkm.tsv'.format(cohort,cohort)
    mirna_path = '../data/TCGA/{}/Omics/miRNA/{}.mirna.tsv'.format(cohort,cohort)
    methyl_path = '../data/TCGA/{}/Omics/Methyl450/{}.methylation450.tsv'.format(cohort,cohort)
    clinical_path = '../data/TCGA/{}/Clinical/{}.GDC_phenotype_slides.tsv'.format(cohort,cohort)
    ...
    clinical_df = clinical_df[clinical_df['slide_id'].notna()]
    ...
    hallmarks = extract_hallmarks(omics_df)
    hallmarks_loc = get_hallmarks_loc(omics_df, hallmarks)
    return omics_df, clinical_df.loc[lt_samples,:], lt_samples, hallmarks, hallmarks_loc
...
def read_data_pancan(omic_sources):
    ...
    for omic_source in omic_sources:
        directory = '../data/TCGA/Pancancer/processed_pancan/{}'.format(omic_source)
        chr = 0
    ...
    with open('../data/TCGA/Pancancer/processed_pancan/clinical/clinical.pkl', 'rb') as clinical_file:
